def normalize_data(n_cases, n_people, scale):
    ...
    for idx, n in enumerate(n_cases):
        norm_cases.append(n_covid[idx] * scale / n_people[idx]) # TODO) Calculate the number of cases per its population
    return norm_cases
...
for idx, pop in enumerate(n_people):
    ratio = pop * 100 / sum_people # TODO) Calculate the ratio of each local population to the total population
...
for idx, pop in enumerate(n_covid):
    ratio = pop * 100 / sum_covid # TODO) Calculate the ratio of each local population to the total population
